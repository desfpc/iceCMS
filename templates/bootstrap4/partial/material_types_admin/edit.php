 ...
 */
?>
    <form id="matTypeAddForm"
          action="/admin/material_types_admin/?mode=edit&id=<?= $this->moduleData->matType->params['id'] ?>"
          method="post"><input type="hidden" name="id" value="<?= $this->moduleData->matType->params['id'] ?>">
    <div class="form-group row">
        <label for="name" class="col-sm-2 col-form-label">Наименование</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" id="name" name="name" aria-describedby="nameHelp"
                   placeholder="Наименование типа материала" required
                   value="<?= $this->moduleData->matType->params['name']; ?>">
        </div>
    </div>
    <div class="form-group row">
        <label for="name_en" class="col-sm-2 col-form-label">Наименование (En)</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" id="name_en" name="name_en" aria-describedby="name_enHelp"
                   placeholder="Наименование (En)" value="<?= $this->moduleData->matType->params['name_en']; ?>">
        </div>
    </div>
    <div class="form-group row">
        <label for="name_de" class="col-sm-2 col-form-label">Наименование (De)</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" id="name_de" name="name_de" aria-describedby="name_deHelp"
                   placeholder="Наименование (De)" value="<?= $this->moduleData->matType->params['name_de']; ?>">
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" id="id_char" name="id_char" aria-describedby="id_charHelp"
                   ...
                   value="<?= $this->moduleData->matType->params['id_char']; ?>">
        </div>
    </div>
    <br/>
    <hr/>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select class="form-control selectpicker" data-live-search="true" id="parent_id" name="parent_id"
                    aria-describedby="parent_idHelp" placeholder="Родительский тип материала">
                <option value="<?= $this->moduleData->matType->params['parent_id']; ?>"
                        selected><?= $this->moduleData->parentName; ?></option>
            </select>
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" id="ordernum" name="ordernum" aria-describedby="ordernumHelp"
                   placeholder="Порядок сортировки" value="<?= $this->moduleData->matType->params['ordernum']; ?>">
        </div>
    </div>
    <div class="form-check">
        <input type="checkbox" class="form-check-input" id="sitemenu" name="sitemenu"
               value="1" <?php if ($this->moduleData->matType->params['sitemenu'] == 1) {
            echo 'checked';
        } ?>>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select class="form-control selectpicker" data-live-search="true" id="template_list" name="template_list"
                    aria-describedby="parent_idHelp" placeholder="Шаблон списка материалов">
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select class="form-control selectpicker" data-live-search="true" id="template_item" name="template_item"
                    aria-describedby="parent_idHelp" placeholder="Шаблон детализации материалов">
                <option value="NULL">нет</option>
                <?php
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select class="form-control selectpicker" data-live-search="true" id="template_admin" name="template_admin"
                    aria-describedby="parent_idHelp" placeholder="Шаблон редактирования материалов">
                <option value="NULL">нет</option>
                <?php
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10"><?php
            ...
            ?>
            <input type="text" class="form-control" id="list_items" name="list_items" aria-describedby="ordernumHelp"
                   placeholder="Кол-во материалов на одной странице списка"
                   value="<?= $this->moduleData->matType->params['list_items'] ?>">
        </div>
    </div>
    <div class="form-check">
        <input type="checkbox" class="form-check-input" id="prepare_list" name="prepare_list"
               value="1" <?php if ($this->moduleData->matType->params['prepare_list'] == 1) {
            echo 'checked';
        } ?>>
        <label for="prepare_list" class="form-check-label">Формирование списка материалов (нет логики в шаблоне)</label>
    </div>
    <div class="form-check">
        <input type="checkbox" class="form-check-input" id="prepare_item" name="prepare_item"
               value="1" <?php if ($this->moduleData->matType->params['prepare_item'] == 1) {
            echo 'checked';
        } ?>>
        <label for="prepare_item" class="form-check-label">Формирование детализации материала (нет логики в
            шаблоне)</label>
    </div>
    <br/>
    <hr/>
    <br/>
    <div class="form-check">
        <input type="checkbox" class="form-check-input" id="shop_ifgood" name="shop_ifgood"
               value="1" <?php if ($this->moduleData->matType->params['shop_ifgood'] == 1) {
            echo 'checked';
        } ?>>
        <label for="shop_ifgood" class="form-check-label">Тип является каталогом товаров</label>
    </div>
    <div class="form-check">
        <input type="checkbox" class="form-check-input" id="shop_ifstore" name="shop_ifstore"
               value="1" <?php if ($this->moduleData->matType->params['shop_ifstore'] == 1) {
            echo 'checked';
        } ?>>
    ...
    <div class="row">
        <div class="col">
            ...
            <form method="post"
                  action="/admin/material_types_admin/?mode=addExtraParam&mtype_id=<?= $this->moduleData->matType->params['id'] ?>">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="ep_name" name="name" aria-describedby="nameHelp"
                               placeholder="Наименование дополнительного поля" required value="">
                    </div>
                </div>
                <div class="form-group row">
                    <label for="ep_value_type" class="col-sm-2 col-form-label">Тип значения</label>
                    <div class="col-sm-10">
                        <select class="form-control selectpicker" data-live-search="false" id="ep_value_type"
                                name="value_type" aria-describedby="ep_value_typeHelp"
                                placeholder="Тип значения параметра">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select class="form-control selectpicker" data-live-search="true" id="ep_value_mtype"
                                name="value_mtype" aria-describedby="ep_value_mtypeHelp"
                                placeholder="Тип материала для значиней">
                            <option value="NULL">нет</option>
                        </select>
                    </div>
                </div>

                <button type="submit" class="btn btn-success"><i class="material-icons md-24 md-light">add_box</i>
                    Создать
                </button>
            </form>
        </div>
